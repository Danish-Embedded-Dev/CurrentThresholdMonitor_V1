...
//--------login-Page--------
  ...
  #define config_timeout 1*60   //10min or 600 seconds timeout for configuration mode
...
//------------EEPROM Block----------
  ...
  #define Reserved_pvt_bytes 125
  #define PAGE_BASE_ADDR 128 
  #define PAGE_SIZE 32
...
//------------------MODULE_CURRENT_EVENT-----------
  ...
  byte TIMER_INTERVAL = 150;  //100ms
   
  bool packet_update = false; //use for packet send to mqtt or not 

  bool alert_status = LOW;   //use for periodic or alert packet

  double LastReading = 0;    //use for current last reading store
